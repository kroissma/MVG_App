...
public abstract class SpringFxApplication extends Application {
    ...
    private String shortenBeanName(String beanName) {
        ...
        for (Iterator<String> iterator = Arrays.asList(beanName.split("\\.")).iterator();
            iterator.hasNext(); ) {
            String splitName = iterator.next();
            shortBeanNameBuilder
                .append(iterator.hasNext() ? (splitName.charAt(0) + ".") : splitName);
        }
    ...
    private class PreloaderNotifyingBeanPostProcessor implements
        ...
        public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) {
            notifyPreloader(new SpringProgressNotification(progress,
                "Instantiating: " + shortenBeanName(beanClass.getName())));
            return null;
        }
