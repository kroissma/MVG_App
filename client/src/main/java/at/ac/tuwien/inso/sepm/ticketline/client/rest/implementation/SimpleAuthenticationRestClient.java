...
public class SimpleAuthenticationRestClient implements AuthenticationRestClient {
    ...
    private static final Logger LOGGER = LoggerFactory
        .getLogger(SimpleAuthenticationRestClient.class);
    ...
    public AuthenticationToken authenticate(final AuthenticationRequest authenticationRequest)
        ...
        try {
            LOGGER.info("Authenticate {} at {}", authenticationRequest.getUsername(),
            ...
            LOGGER.debug("Authenticate {} status {}", authenticationRequest.getUsername(),
                response.getStatusCode());
            return response.getBody();
        } catch (HttpStatusCodeException e) {
            throw new DataAccessException(
                "Failed to login with status code " + e.getStatusCode().toString(), e);
        } catch (ResourceAccessException e) {
            throw new DataAccessException("Unable to connect to server", e);
    ...
    public AuthenticationToken authenticate() throws DataAccessException {
        try {
            LOGGER.info("Get AuthenticationToken at {}",
                restClient.getServiceURI(AUTHENTICATION_URL));
            ResponseEntity<AuthenticationToken> response =
                restClient.exchange(
                    restClient.getServiceURI(AUTHENTICATION_URL),
    ...
    public AuthenticationTokenInfo tokenInfoCurrent() throws DataAccessException {
        try {
            LOGGER.info("Get AuthenticationTokenInfo at {}",
